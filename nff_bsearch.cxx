...
//	double approx_time = approximate_initial_belief();
    ...
    if ( models().empty() )
    {
        std::cout << "[搜索] 生成初始样本集..." << std::endl;
        approx_time = approximate_initial_belief();
        std::cout << "[搜索] 初始样本生成完成: |S|=" << models().size() << std::endl;
    }
    else
    {
        std::cout << "[搜索] 复用已有样本集: |S|=" << models().size() << std::endl;
        sm_task.set_initial_models( &(m_models) );
    }
    ...
    log.stream() << ";; Plan found:" << std::endl;
        ...
        std::cout << "[搜索] 候选规划提取完成, 长度=" << m_last_plan.size() << std::endl;
